...
class PatentBasicMetric(BaseModel):
    ...
    def calc(cls, _id: str, name: str, df: pd.DataFrame) -> List['PatentBasicMetric']:
        ...
        计算专利相关基础指标
        """
        mask_0_5_year = (END_YEAR - 10 < df["申请年"]) & (df["申请年"] <= END_YEAR - 5)  # 2015-2019
        mask_5_10_year = (END_YEAR - 5 < df["申请年"]) & (df["申请年"] <= END_YEAR)  # 2020-2024
        results = []
        for time_window, mask_time_window in zip([0, 1], [mask_0_5_year, mask_5_10_year]):
            ...
            df_res = df_sub.drop_duplicates(subset=["DWPI 入藏号"])
